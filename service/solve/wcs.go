...
func (s *server) solveForWCSFITSHandler(ctx context.Context, req *connect.Request[pb.SolveForWCSFITSHandlerRequest]) (*connect.Response[pb.SolveForWCSFITSHandlerResponse], error) {
	...
	if !exists {
		s.Logger.Error().Msg("Failed to get RA header")
		return nil, fmt.Errorf("ra header not found")
	}

	// Attempt to get the Dec header from the FITS file:
	dec, exists := fit.Header.Floats["DEC"]
	if !exists {
		s.Logger.Error().Msg("Failed to get Dec header")
		return nil, fmt.Errorf("dec header not found")
	}
